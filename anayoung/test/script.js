...
function keyHtml(song) {
    ...
    if (isNaN(num) || num === 0) return '<span class="key-none">-</span>';
    if (num > 0) return `<span class="key-up">+${num}</span>`;
...
function renderProbabilityHtml(item) {
    ...
    return `<div class="probability-wrap">` +
        `<button type="button" class="probability-trigger" aria-expanded="false" aria-label="${esc(item.label)} 확률 확인">${esc(item.probabilityTrigger || '🎈')}</button>` +
        ...
        li.innerHTML =
            `<div class="info-item-line">` +
                ...
                `<div class="info-item-meta">` +
                    ...
                    `${hasProbability ? renderProbabilityHtml(item) : ''}` +
                `</div>` +
            `</div>` +
...
function openLyricsModal(song) {
    ...
    if (keyEl) {
        ...
        if (!isNaN(k) && k !== 0) {
            keyEl.innerHTML = k > 0
                ? `<span class="key-up">키 +${k}</span>`
                : `<span class="key-down">키 ${k}</span>`;
            keyEl.style.display = '';
        } else {
...
function weightedRandomPick(pool) {
    ...
    const EXPONENT = 0.3;
    const weights  = pool.map(song => 1 / Math.pow((song.sungCount || 0) + 1, EXPONENT));
...
function pickRandom() {
    ...
    if (randomKeyEl) {
        const k = parseInt(song.key, 10);
        if (!isNaN(k) && k !== 0) {
            randomKeyEl.innerHTML = k > 0
                ? `<span class="key-up">키 +${k}</span>`
                : `<span class="key-down">키 ${k}</span>`;
            randomKeyEl.style.display = '';
        } else {
            randomKeyEl.style.display = 'none';
        }
    }
