...
const SONG_CSV_URL =
    ...
    + `/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(CONFIG.SONG_SHEET)}`
...
let pendingFavSong = null;       // 로그인 후 자동 처리할 곡
let pendingAction = null;        // 'fav' | 'viewFavorites' | 'save'
...
function cacheDom() {
    DOM = {
        loginOverlay: $('#loginOverlay'), createOverlay: $('#createOverlay'),
        userIdInput: $('#userIdInput'), loginBtn: $('#loginBtn'),
        loginError: $('#loginError'), loginLoading: $('#loginLoading'),
        createIdDisplay: $('#createIdDisplay'),
        createConfirm: $('#createConfirmBtn'), createCancel: $('#createCancelBtn'),
        createLoading: $('#createLoading'), createError: $('#createError'),
        loginOpenBtn: $('#loginOpenBtn'),
        userBadge: $('#userBadge'), saveFavBtn: $('#saveFavBtn'), logoutBtn: $('#logoutBtn'),
        tbody: $('#songTableBody'), totalCount: $('#totalCount'),
        filteredCount: $('#filteredCount'), favCount: $('#favCount'),
        noResult: $('#noResult'), loading: $('#loading'),
        errorMsg: $('#errorMsg'), wrapper: $('#songListWrapper'),
        searchInput: $('#searchInput'), sentinel: $('#scrollSentinel'),
        loadingMore: $('#loadingMore'), lyricsModal: $('#lyricsModal'),
        randomModal: $('#randomModal'), saveModal: $('#saveConfirmModal'),
        toast: $('#toast'), scrollTopBtn: $('#scrollTopBtn'),
    };
}
...
/* ══════ API ══════ */
async function apiGet(a, id) {
    const r = await fetch(`${CONFIG.API_URL}?action=${encodeURIComponent(a)}&id=${encodeURIComponent(id)}`);
    if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json();
}
async function apiPost(a, id, x = {}) {
    const r = await fetch(CONFIG.API_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: JSON.stringify({ action: a, id, ...x }) });
    if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json();
}
...
function hideLoginOverlay() {
    ...
    pendingAction = null; pendingFavSong = null;
}
...
function setLoginLoading(on) {
    ...
    DOM.loginBtn.disabled = on; DOM.userIdInput.disabled = on;
}
...
async function handleLogin() {
    ...
    if (!/^[a-zA-Z0-9가-힣_\-]{1,20}$/.test(id)) {
        showLoginError('영문, 한글, 숫자, _, - 만 사용 (1~20자)'); return;
    }
    ...
    try {
        ...
        if (result.exists) { await loadAndLogin(id); }
        else { setLoginLoading(false); showCreateOverlay(id); }
    } catch (e) {
        console.error(e); showLoginError('서버에 연결할 수 없습니다'); setLoginLoading(false);
    }
}
...
async function loadAndLogin(id) {
    try {
        ...
        serverFavKeys = new Set(favorites); favDirty = false;
        localStorage.setItem(CONFIG.USER_KEY, id);
        setLoginLoading(false); hideLoginOverlay(); updateUserBar();
        showToast(`👋 ${id}님 환영합니다! (즐겨찾기 ${result.count}곡)`);
        executePendingAction();
        filterAndRender();
    } catch (e) {
        console.error(e); showLoginError('즐겨찾기를 불러올 수 없습니다'); setLoginLoading(false);
    }
}
...
function showCreateOverlay(id) {
    ...
    DOM.createError.style.display = 'none'; DOM.createLoading.style.display = 'none';
    DOM.createConfirm.disabled = false; DOM.createCancel.disabled = false;
    DOM.loginOverlay.classList.remove('active');
...
async function handleCreateConfirm() {
    ...
    DOM.createConfirm.disabled = true; DOM.createCancel.disabled = true;
    DOM.createLoading.style.display = 'flex'; DOM.createError.style.display = 'none';
    try {
        const result = await apiPost('create', id);
        if (result.success) {
            currentUser = id; favorites = new Set(); serverFavKeys = new Set(); favDirty = false;
            localStorage.setItem(CONFIG.USER_KEY, id);
            DOM.createLoading.style.display = 'none'; hideCreateOverlay(); updateUserBar();
            ...
            DOM.createConfirm.disabled = false; DOM.createCancel.disabled = false;
            DOM.createError.textContent = result.error || '생성 실패'; DOM.createError.style.display = 'block';
        }
    } catch (e) {
        console.error(e); DOM.createLoading.style.display = 'none';
        DOM.createConfirm.disabled = false; DOM.createCancel.disabled = false;
        DOM.createError.textContent = '서버 연결 실패'; DOM.createError.style.display = 'block';
    }
}
...
function handleLogout() {
    if (favDirty && currentUser) { if (!confirm('저장하지 않은 변경사항이 있습니다.\n정말 로그아웃하시겠습니까?')) return; }
    currentUser = null; favorites = new Set(); serverFavKeys = new Set(); favDirty = false;
    localStorage.removeItem(CONFIG.USER_KEY);
    updateUserBar();
    if (activeView === 'favorites') {
        activeView = 'all';
        $('#viewFilters').querySelectorAll('.filter-btn').forEach(b => b.classList.toggle('active', b.dataset.value === 'all'));
    }
    filterAndRender();
    showToast('👋 로그아웃되었습니다');
}
...
async function tryAutoLogin() {
    const saved = localStorage.getItem(CONFIG.USER_KEY); if (!saved) return;
    try {
...
function toggleFavorite(song) {
    ...
    const key = songKey(song), wasFav = favorites.has(key);
    ...
    else DOM.tbody.querySelectorAll(`.fav-btn[data-key="${CSS.escape(key)}"]`).forEach(b => {
        b.textContent = !wasFav ? '❤️' : '🤍'; b.classList.toggle('active', !wasFav);
    });
}
...
async function handleSave() {
    if (!currentUser) return;
    const ld = $('#saveLoading'); ld.style.display = 'flex'; $('#saveConfirmBtn').disabled = true;
    try {
        ...
        if (result.success) {
            serverFavKeys = new Set(favorites); favDirty = false; updateSaveBtn();
            closeSaveModal(); showToast(`💾 ${result.count}곡 저장 완료!`);
        } else showToast(`❌ ${result.error}`);
    } catch { showToast('❌ 서버 연결 실패'); }
    ld.style.display = 'none'; $('#saveConfirmBtn').disabled = false;
}

/* ══════ 즐겨찾기 필터 클릭 가드 ══════ */
function handleViewFilter(value) {
    if (value === 'favorites' && !currentUser) {
        requireLogin('viewFavorites');
        return;
    }
    activeView = value;
    filterAndRender();
...
/* ══════ 테마 ══════ */
function loadTheme() {
    const s = localStorage.getItem(CONFIG.THEME_KEY);
    const p = window.matchMedia?.('(prefers-color-scheme:light)').matches ? 'light' : 'dark';
    applyTheme(s || p);
}
...
function hilite(t, q) {
    if (!q) return esc(t); let lo = t.toLowerCase(), lq = q.toLowerCase(), i = lo.indexOf(lq);
...
async function fetchSongs() {
    try {
        ...
        let csv; const c = sessionStorage.getItem(CONFIG.CACHE_KEY);
        ...
        const p = Papa.parse(csv, { header: true, skipEmptyLines: true, transformHeader: h => h.trim() }); const h = p.meta.fields || [];
        ...
        DOM.loading.style.display = 'none'; DOM.wrapper.style.display = 'block'; filterAndRender();
...
function filterAndRender() {
    ...
    DOM.totalCount.textContent = allSongs.length; DOM.filteredCount.textContent = filteredSongs.length;
...
function renderMore() {
    ...
    requestAnimationFrame(() => {
        ...
        for (let i = displayedCount; i < end; i++) {
            ...
            tr.innerHTML = `<td>${i + 1}</td><td><button class="fav-btn${isFav ? ' active' : ''}" data-key="${esc(key)}" data-idx="${i}">${isFav ? '❤️' : '🤍'}</button></td><td><div class="title-cell"><span class="song-title">${hilite(s.title, searchQuery)}</span><button class="lyrics-btn ${hasL ? 'has-lyrics' : 'no-lyrics'}" data-idx="${i}">📜</button></div></td><td class="song-artist">${hilite(s.artist, searchQuery)}</td><td>${gB}</td><td><span class="badge ${gC}">${gI} ${esc(s.gender)}</span></td>`;
            frag.appendChild(tr);
        }
...
function openLyricsModal(s) {
    ...
    const g = $('#modalGender'); g.textContent = (s.gender === '여' ? '👩 ' : '👨 ') + s.gender;
    g.className = `mtag ${s.gender === '여' ? 'gender-tag' : 'gender-tag-m'}`;
    $('#modalLyrics').textContent = s.lyrics.replace(/^[""\u201C\u201D]|[""\u201C\u201D]$/g, '').trim() || '가사 정보가 없습니다.';
...
function pickRandom() {
    ...
    const g = $('#randomGender'); g.textContent = (s.gender === '여' ? '👩 ' : '👨 ') + s.gender;
    g.className = `mtag ${s.gender === '여' ? 'gender-tag' : 'gender-tag-m'}`;
...
document.addEventListener('DOMContentLoaded', () => {
    cacheDom(); loadTheme(); updateSortUI(); initScrollObserver(); initScrollTop();
    updateUserBar();
    ...
    $('#viewFilters').addEventListener('click', e => {
        ...
        $('#viewFilters').querySelectorAll('.filter-btn').forEach(x => x.classList.remove('active'));
    ...
    $('#genderFilters').addEventListener('click', e => {
        const b = e.target.closest('.filter-btn'); if (!b) return;
        $('#genderFilters').querySelectorAll('.filter-btn').forEach(x => x.classList.remove('active'));
        b.classList.add('active'); activeGender = b.dataset.value; filterAndRender();
    });
